...
fun main(args: Array<String>) {
    ...
    Paths.get(System.getProperty("user.dir"), "items.xlsx").toFile().let(WorkbookFactory::create).let { workbook ->
        val items = workbook["main"]?.rowIterator()?.asSequence()?.filter { it.rowNum != 0 }?.map {
            (it[config["main"]?.get("file") ?: 7]?.stringCellValue ?: "default") to JsonObject().apply {
                ...
                it.value.map {
                    it.apply {
                        elements.map { element ->
                            element.value[get("id").asString]?.let { element.key to it }
